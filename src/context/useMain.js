import { useState } from "react";
const useMain = () => {
  // ca
  const [hdMoBlObj, setHdMoBlObj] = useState(null);
  const [hdMoObj, setHdMoObj] = useState(null);
  const [cdMoObj, setCdMoObj] = useState(null);
  const [hdPoBlObj, setHdPoBlObj] = useState(null);
  const [hdPoObj, setHdPoObj] = useState(null);
  const [cdPoObj, setCdPoObj] = useState(null);
  // const [cd, setCd] = useState(null);
  const [toReviseAdj, setToReviseAdj] = useState(null);

  //tg
  const [HoverLocktg, setHoverLocktg] = useState(null);

  // current adj
  const [currCapAc, setCurrCapAc] = useState("none");
  const [currCapTg, setCurrCapTg] = useState("none");

  // selected
  const [sdTgAdjNou, setSdTgAdjNou] = useState(null);
  // ac
  const [cdAcObj, setCdAcObj] = useState(null);

  // hovered
  const [hdAdjNouTg, setHdAdjNouTg] = useState(null);
  const [hdAdjNouAc, setHdAdjNouAc] = useState(null);
  // enter
  const [isEnterSelected, setisEnterSelected] = useState(false);
  return {
    cdPoObj,
    setCdPoObj,
    hdPoObj,
    setHdPoObj,
    cdAcObj,
    setCdAcObj,
    // cd,
    // setCd,
    hdPoBlObj,
    setHdPoBlObj,
    toReviseAdj,
    setToReviseAdj,
    hdMoBlObj,
    setHdMoBlObj,
    currCapAc,
    setCurrCapAc,
    currCapTg,
    setCurrCapTg,
    sdTgAdjNou,
    setSdTgAdjNou,
    hdAdjNouTg,
    setHdAdjNouTg,
    hdAdjNouAc,
    setHdAdjNouAc,
    isEnterSelected,
    setisEnterSelected,
    HoverLocktg,
    setHoverLocktg,
    cdMoObj,
    setCdMoObj,
    hdMoObj,
    setHdMoObj,
  };
};
export default useMain;
